...
module Neofiles
  ...
  def resized_image_dimensions(image_file, width, height, resize_options)
    ...
    # сначала получим и проверим переданный объект
    ...
    command.size(sprintf('%dx%d', image_file.width, image_file.height))   # габариты входного файла
    command.xc('white')                                                   # входной файл число белый
    command.resize(sprintf('%dx%d', width, height))                       # изменить размер до нужного
  ...
  module_function :resized_image_dimensions, :crop_requested?
end